...
import com.sina.sinawidgetdemo.utils.NetworkUtils;
...
public class ReuqestDataProcess {
   ...
   public static void requestData(boolean isRefresh,int page,RequestModel requestModel,RequestOptions requestOptions,
	...
	public static void requestData(boolean isRefresh,
		...
		TaskModel taskModel = creatTaskModel(isRefresh, Integer.MAX_VALUE,
				requestModel, requestOptions);
		EngineManager.getInstance().getRequestDataTaskManager()
				.startTask(taskModel, requestDataListener, dbLogicInterface);
	}
...
//		taskModel.setPartner_id(RequestConstant.FILTER_PARTNER_ID_VALUE);
...
//			        .addTaskOrder(TaskTypeEnum.getNet);
		    		...
		    		if(NetworkUtils.isNetworkAvailable(EngineManager.getInstance().getContext())){
			    		taskModel.addTaskOrder(TaskTypeEnum.getNet);
			    	}
			    	else {
			    	    taskModel.addTaskOrder(TaskTypeEnum.getDb);
