package com.sina.sinawidgetdemo.custom.autoscrollviewpager;


import java.lang.ref.WeakReference;
import java.lang.reflect.Field;

import com.sina.sinawidgetdemo.custom.view.CustomDurationScroller;
...
public class AutoScrollViewPager extends ViewPager {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            downPositionX = ev.getX();
            downPositionY = ev.getY();
        }
        ...
        if (judgeHOrV(downPositionX, downPositionY, touchX, touchY) == 0) {
            getParent().requestDisallowInterceptTouchEvent(true);
        } else {
            getParent().requestDisallowInterceptTouchEvent(false);
        }
     ...
     * @return 0->横向 1>纵向
     */
    private int judgeHOrV(float downx, float downy, float currentx, float currenty) {
